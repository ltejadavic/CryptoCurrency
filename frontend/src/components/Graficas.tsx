...
const Graficas: React.FC = () => {
      ...
      .then(response => {
        ...
        if (preciosCriptos) {
          ...
          response.data.forEach((portafolio: Portafolio) => {
            ...
            if (precioActual) {
              chartData.push({
                labels: ['Precio de Compra', 'Precio Actual'],
                datasets: [{
  ...
  return (
    <div>
      <h2>Gráficas de Comparación</h2>
      {data.length > 0 ? (
        data.map((chartData, index) => (
          <div key={index}>
            <h3>Gráfico para {portafolios[index].criptomoneda}</h3>
            <Line data={chartData} />
          </div>
        ))
      ) : (
        <p>Cargando datos de las criptomonedas...</p>
      )}
